...
contract WorkflowRegistryManager is Ownable2StepMsgSender, ITypeAndVersion {
    ...
    function addVersion(address contractAddress, uint64 chainID, uint32 deployedAt, bool autoActivate)
        external
        onlyOwner
    {
    ...
    function getVersionNumberByContractAddressAndChainID(address contractAddress, uint64 chainID)
        external
        view
        returns (uint32 versionNumber)
    {
    ...
    /// @custom:throws InvalidContractAddress if the address is zero or contains no code.
    function _validateContractAddress(address _addr) internal view {
