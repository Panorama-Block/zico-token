...
contract WorkflowRegistryManager_activateVersion is WorkflowRegistryManagerSetup {
    ...
    function test_WhenTheVersionNumberIsNotActive_AndWhenThereAreNoActiveVersions() external {
        ...
        vm.expectEmit(true, true, false, true);
        emit WorkflowRegistryManager.VersionActivated(
            address(s_mockWorkflowRegistryContract), s_chainID, newVersionNumber
        );
        emit WorkflowRegistryManager.VersionDeactivated(
            address(s_mockWorkflowRegistryContract), s_chainID, versionNumber
        );
