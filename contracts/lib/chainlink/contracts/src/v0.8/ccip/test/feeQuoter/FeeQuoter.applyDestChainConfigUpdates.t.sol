...
contract FeeQuoter_applyDestChainConfigUpdates is FeeQuoterSetup {
    ...
    function testFuzz_applyDestChainConfigUpdates_Success(FeeQuoter.DestChainConfigArgs memory destChainConfigArgs)
        public
    {
        ...
        for (uint256 i = 0; i < s_validChainFamilySelector.length; i++) {
            ...
            destChainConfigArgs.destChainSelector = uint64(
                uint256(
                    keccak256(abi.encodePacked(destChainConfigArgs.destChainSelector, s_validChainFamilySelector[i]))
                )
            );
            ...
            _assertFeeQuoterDestChainConfigsEqual(
                destChainConfigArgs.destChainConfig,
                s_feeQuoter.getDestChainConfig(destChainConfigArgs.destChainSelector)
            );
        }
    }
