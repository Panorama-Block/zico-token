...
contract FeeQuoter_getValidatedFee is FeeQuoterFeeSetup {
    ...
    function test_getValidatedFee_HighGasMessage() public view {
        ...
        for (uint256 i = 0; i < feeTokenPrices.length; ++i) {
            ...
            uint256 callDataCostHigh = (customDataSize - DEST_GAS_PER_PAYLOAD_BYTE_THRESHOLD)
                * DEST_GAS_PER_PAYLOAD_BYTE_HIGH + DEST_GAS_PER_PAYLOAD_BYTE_THRESHOLD * DEST_GAS_PER_PAYLOAD_BYTE_BASE;
    ...
    function test_getValidatedFee_SingleTokenMessage() public view {
        ...
        for (uint256 i = 0; i < feeTokenPrices.length; ++i) {
            ...
            message.feeToken = testTokens[i];
            FeeQuoter.DestChainConfig memory destChainConfig = s_feeQuoter.getDestChainConfig(DEST_CHAIN_SELECTOR);
            uint32 destBytesOverhead = s_feeQuoter.getTokenTransferFeeConfig(
                DEST_CHAIN_SELECTOR, message.tokenAmounts[0].token
            ).destBytesOverhead;
    ...
    function test_getValidatedFee_MessageWithDataAndTokenTransfer() public view {
        ...
        for (uint256 i = 0; i < feeTokenPrices.length; ++i) {
            ...
            (uint256 transferFeeUSD,, uint256 tokenTransferBytesOverhead) = s_feeQuoter.getTokenTransferCost(
                DEST_CHAIN_SELECTOR, message.feeToken, feeTokenPrices[i], message.tokenAmounts
            );
    ...
    function test_getValidatedFee_RevertWhen_TooManyTokens() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(FeeQuoter.UnsupportedNumberOfTokens.selector, tooMany, MAX_TOKENS_LENGTH)
        );
        s_feeQuoter.getValidatedFee(DEST_CHAIN_SELECTOR, message);
    }
    ...
    function test_getValidatedFee_RevertWhen_InvalidSVMExtraArgsWritableBitmap() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(FeeQuoter.InvalidSVMExtraArgsWritableBitmap.selector, wrongBitmap, accounts)
        );
        s_feeQuoter.getValidatedFee(DEST_CHAIN_SELECTOR, message);
    }
}
