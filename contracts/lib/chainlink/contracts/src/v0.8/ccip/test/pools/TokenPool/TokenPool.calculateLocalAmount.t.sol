...
contract TokenPool_calculateLocalAmount is TokenPoolSetup {
    ...
    function test_RevertWhen_calculateLocalAmountWhen_LowRemoteDecimalsOverflows() public {
        ...
        vm.mockCall(
            address(s_token), abi.encodeWithSelector(IERC20Metadata.decimals.selector), abi.encode(localDecimals)
        );

        s_tokenPool = new TokenPoolHelper(
            s_token, localDecimals, new address[](0), address(s_mockRMNRemote), address(s_sourceRouter)
        );
