...
contract FeeQuoterHelper is FeeQuoter {
    ...
    function parseEVMExtraArgsFromBytes(bytes calldata extraArgs, uint64 destChainSelector, bool enforceOutOfOrder)
        external
        view
        returns (Client.GenericExtraArgsV2 memory)
    {
    ...
    function parseSVMExtraArgsFromBytes(bytes calldata extraArgs, DestChainConfig memory destChainConfig)
        external
        pure
        returns (Client.SVMExtraArgsV1 memory)
    {
        return
    ...
    function validateDestFamilyAddress(bytes4 chainFamilySelector, bytes memory destAddress, uint256 gasLimit)
        external
        pure
    {
