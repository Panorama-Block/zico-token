...
contract FeeQuoter_applyTokenTransferFeeConfigUpdates is FeeQuoterSetup {
    function testFuzz_ApplyTokenTransferFeeConfig_Success(
        ...
        tokenTransferFeeConfigs[0].maxFeeUSDCents = uint32(
            bound(
                tokenTransferFeeConfigs[0].maxFeeUSDCents,
                tokenTransferFeeConfigs[0].minFeeUSDCents + 1,
                type(uint32).max
            )
        );
        tokenTransferFeeConfigs[1].maxFeeUSDCents = uint32(
            bound(
                tokenTransferFeeConfigs[1].maxFeeUSDCents,
                tokenTransferFeeConfigs[1].minFeeUSDCents + 1,
                type(uint32).max
            )
        );
    ...
    function test_ApplyTokenTransferFeeConfig() public {
        ...
        tokenTransferFeeConfigArgs[0].tokenTransferFeeConfigs[0].tokenTransferFeeConfig = FeeQuoter
        ...
        tokenTransferFeeConfigArgs[0].tokenTransferFeeConfigs[1].tokenTransferFeeConfig = FeeQuoter
            .TokenTransferFeeConfig({
            minFeeUSDCents: 12,
            maxFeeUSDCents: 13,
            deciBps: 14,
