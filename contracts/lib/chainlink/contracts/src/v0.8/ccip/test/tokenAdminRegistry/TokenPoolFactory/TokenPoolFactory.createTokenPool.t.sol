...
contract TokenPoolFactory_createTokenPool is TokenPoolFactorySetup {
    ...
    function test_createTokenPool_WithNoExistingRemoteContracts_predict() public {
        ...
        address predictedTokenAddress =
            dynamicSalt.computeAddress(keccak256(s_tokenInitCode), address(newTokenPoolFactory));
...

        {
            ...
            bytes memory predictedPoolCreationParams = abi.encode(
                predictedTokenAddress, LOCAL_TOKEN_DECIMALS, new address[](0), s_rmnProxy, address(s_destRouter)
            );
        ...
        assertEq(
            s_tokenAdminRegistry.getTokenConfig(tokenAddress).pendingAdministrator,
            address(0),
    ...
    function test_createTokenPool_ExistingRemoteToken_AndPredictPool() public {
        ...
        // is used in the constructor params
        bytes memory predictedPoolCreationParams = abi.encode(
            address(newRemoteToken), LOCAL_TOKEN_DECIMALS, new address[](0), s_rmnProxy, address(s_destRouter)
        );
    ...
    function test_createTokenPool_RemoteTokenHasDifferentDecimals() public {
        ...
        // is used in the constructor params
        bytes memory predictedPoolCreationParams = abi.encode(
            address(newRemoteToken), REMOTE_TOKEN_DECIMALS, new address[](0), s_rmnProxy, address(s_destRouter)
        );
        ...
        assertEq(
            TokenPool(poolAddress).getTokenDecimals(), LOCAL_TOKEN_DECIMALS, "Local token pool should use 18 decimals"
        );
