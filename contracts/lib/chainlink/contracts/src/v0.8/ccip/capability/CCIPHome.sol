...
contract CCIPHome is Ownable2StepMsgSender, ITypeAndVersion, ICapabilityConfiguration, IERC165 {
    ...
    function getCapabilityConfiguration(uint32) external pure override returns (bytes memory configuration) {
        return bytes("");
    }
    ...
    function applyChainConfigUpdates(uint64[] calldata chainSelectorRemoves, ChainConfigArgs[] calldata chainConfigAdds)
        external
        onlyOwner
    {
