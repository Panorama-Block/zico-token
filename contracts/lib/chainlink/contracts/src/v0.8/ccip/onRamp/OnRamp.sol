...
contract OnRamp is IEVM2AnyOnRampClient, ITypeAndVersion, Ownable2StepMsgSender {
    ...
    function _postProcessMessage(Internal.EVM2AnyRampMessage memory message)
        internal
        virtual
    ...
    /// configured.
    function getAllowedSendersList(uint64 destChainSelector)
        external
        view
        returns (bool isEnabled, address[] memory configuredAddresses)
    {
        return (
