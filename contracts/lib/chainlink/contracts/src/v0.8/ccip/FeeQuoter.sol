...
contract FeeQuoter is AuthorizedCallers, IFeeQuoter, ITypeAndVersion, IReceiver, KeystoneFeedsPermissionHandler {
    ...
    /// @return gasPriceValue The price of gas in 1e18 dollars per base unit.
    function _getValidatedGasPrice(uint64 destChainSelector, uint32 gasPriceStalenessThreshold)
        private
        view
        returns (uint224 gasPriceValue)
    {
    ...
    /// to calculate fees.
    function applyFeeTokensUpdates(address[] memory feeTokensToRemove, address[] memory feeTokensToAdd)
        external
        onlyOwner
    {
        ...
        returns (uint256 feeTokenAmount)
    {
        ...
        if (calldataLength > destChainConfig.destGasPerPayloadByteThreshold) {
            destCallDataCost = destChainConfig.destGasPerPayloadByteBase
                * destChainConfig.destGasPerPayloadByteThreshold
                + (calldataLength - destChainConfig.destGasPerPayloadByteThreshold)
    ...
    function _getDataAvailabilityCost(
        ...
        return ((dataAvailabilityGas * dataAvailabilityGasPrice) * destChainConfig.destDataAvailabilityMultiplierBps)
            * 1e14;
    }
    ...
    function _validateDestFamilyAddress(bytes4 chainFamilySelector, bytes memory destAddress, uint256 gasLimit)
        internal
        pure
    {
    ...
    function _parseSVMExtraArgsFromBytes(bytes calldata extraArgs, uint256 maxPerMsgGasLimit, bool enforceOutOfOrder)
        internal
        pure
        returns (Client.SVMExtraArgsV1 memory svmExtraArgs)
    {
    ...
    function _parseUnvalidatedEVMExtraArgsFromBytes(bytes calldata extraArgs, uint64 defaultTxGasLimit)
        private
        pure
        returns (Client.GenericExtraArgsV2 memory)
    {
        if (extraArgs.length == 0) {
        ...
        if (extraArgsTag == Client.GENERIC_EXTRA_ARGS_V2_TAG) {
            ...
            return
                Client.GenericExtraArgsV2({gasLimit: abi.decode(argsData, (uint256)), allowOutOfOrderExecution: false});
        }
        revert InvalidExtraArgsTag();
    }
        ...
        if (
            ...
            if (svmExtraArgsV1.accountIsWritableBitmap >> svmExtraArgsV1.accounts.length != 0) {
                revert InvalidSVMExtraArgsWritableBitmap(
                    svmExtraArgsV1.accountIsWritableBitmap, svmExtraArgsV1.accounts.length
                );
            }
    ...
    function _processChainFamilySelector(
        ...
        internal
        view
        returns (bytes memory validatedExtraArgs, bool allowOutOfOrderExecution, bytes memory tokenReceiver)
    {
