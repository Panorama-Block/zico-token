...
contract DataFeedsLegacyAggregatorProxy is AggregatorV2V3Interface, ConfirmedOwner {
    ...
    /// instead which includes better verification information.
    function getAnswer(uint256 roundId) external view virtual override returns (int256 answer) {
    ...
    /// an already answered Aggregator or use the recommended getRoundData
    /// instead which includes better verification information.
    function getTimestamp(uint256 roundId) external view virtual override returns (uint256 updatedAt) {
        if (roundId > MAX_ID) return 0;
