...
contract Transmit is SetUp {
    ...
    function test_whenFirstUpkeepFails_subsequentUpkeepsPerform() external {
        ...
        require(
            prevUpkeepBalances[0] == registry.getBalance(linkUpkeepID), "link upkeep balance should remain the same"
        );
        require(
            prevUpkeepBalances[1] > registry.getBalance(linkUpkeepID2), "link upkeep 2 balance should have decreased"
        );
        require(
    ...
    function test_whenSecondUpkeepFails_FirstAndThirdUpkeepsPerform() external {
        ...
        // the first upkeep fails and the second and third upkeep succeeds
        require(
            prevUpkeepBalances[0] > registry.getBalance(nativeUpkeepID), "native upkeep balance should have decreased"
        );
        require(
            prevUpkeepBalances[1] == registry.getBalance(linkUpkeepID), "link upkeep balance should remain the same"
        );
        require(
            prevUpkeepBalances[2] > registry.getBalance(linkUpkeepID2), "link upkeep 2 balance should have decreased"
        );
        // assert token balances have not changed
