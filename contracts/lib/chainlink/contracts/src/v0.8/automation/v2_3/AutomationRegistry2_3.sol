    ...
    function _handleReport(HotVars memory hotVars, Report memory report, uint256 gasOverhead) private {
        ...
        for (uint256 i = 0; i < report.upkeepIds.length; i++) {
            ...
            upkeepTransmitInfo[i].calldataWeight = report.performDatas[i].length
                + TRANSMIT_CALLDATA_FIXED_BYTES_OVERHEAD + (TRANSMIT_CALLDATA_PER_SIGNER_BYTES_OVERHEAD * (hotVars.f + 1));
...

        {
            ...
            for (uint256 i = 0; i < report.upkeepIds.length; i++) {
                if (upkeepTransmitInfo[i].earlyChecksPassed) {
                    billingTokenParams =
                        _getBillingTokenPaymentParams(hotVars, upkeepTransmitInfo[i].upkeep.billingToken);
